    ...
    return (
      <div className="min-h-[80vh] flex items-center justify-center bg-surface-50 py-12 px-4">
        <div className="max-w-md w-full text-center animate-scale-in">
          <div className="w-16 h-16 rounded-full border-2 border-green-300 bg-green-50 flex items-center justify-center mx-auto mb-6">
            <CheckCircle className="w-7 h-7 text-green-600" />
          </div>
          <h2 className="text-2xl font-light text-primary-600 mb-3 tracking-tight">Account Created</h2>
          <p className="text-neutral-500 text-sm mb-4 leading-relaxed">
            We've sent a confirmation email to <strong className="text-primary-600">{email}</strong>.
    ...
    <div className="min-h-[80vh] flex items-center justify-center bg-surface-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full animate-fade-in">
        <div className="text-center mb-10">
          ...
          <h2 className="mt-8 text-3xl font-light text-primary-600 tracking-tight">Create your account</h2>
          <p className="mt-2 text-neutral-500 text-sm">Start your home loan journey today</p>
        </div>
        ...
        <div className="card animate-fade-in-up animate-stagger-2">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  {passwordRequirements.map((req, index) => (
                    <div key={index} className="flex items-center text-xs">
                      {req.valid ? (
                        ...
                        <div className="w-3.5 h-3.5 rounded-full border border-neutral-300 mr-2" />
                      )}
                      <span className={req.valid ? 'text-green-600' : 'text-neutral-400'}>
                        {req.label}
                      </span>
                    </div>
                  ))}
            ...
            <div className="flex items-start">
              ...
              <label htmlFor="terms" className="ml-2 text-sm text-neutral-500">
                I agree to the{' '}
                <Link to="/terms" className="text-accent-600 hover:text-accent-700 transition-colors duration-300">
                  Terms of Service
                </Link>{' '}
                and{' '}
                <Link to="/privacy" className="text-accent-600 hover:text-accent-700 transition-colors duration-300">
                  Privacy Policy
                </Link>
              </label>
            </div>
          ...
          <div className="mt-8 text-center border-t border-neutral-200 pt-6">
            <p className="text-neutral-500 text-sm">
              ...
              <Link to="/login" className="text-primary-600 hover:text-accent-600 font-medium transition-colors duration-300">
                Sign in
              </Link>
            </p>
          </div>
